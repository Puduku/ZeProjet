...
// CHECK_READ_FUNCTION
static int TrivialCheckRead (void *nr_checkReadHandle,
                             const char *p_readBuffer,
                             int readLength,
                             int attemptsCount,
                             int *ac_messageLength) {
  m_DIGGY_BOLLARD()
  m_TRACK_IF(nr_checkReadHandle == NULL)
  m_TRACK_IF(((RDH_HANDLE)nr_checkReadHandle)->bufferCurrentSize < readLength)

  if (((RDH_HANDLE)nr_checkReadHandle)->bufferCurrentSize == readLength) {
...
static int RdhCreateInstance (RDH_HANDLE *azh_handle,
                              int readBufferSize,
                              char *nf_traditionalReadBuffer, 
                              CHECK_READ_FUNCTION n_checkReadFunction,
  ...
  if (n_checkReadFunction != NULL) {
    ...
    ha_rdh->nr_checkReadHandle = ha_rdh ;
  } // if
...
// - RETURNED:
// - -1: unexpected problem
static int RdhPrepareRead (RDH_HANDLE handle, 
                           char **anf_newReadBuffer, int *ac_newReadBufferSize) {
  m_DIGGY_BOLLARD_S()
...
static int RdhCheckRead (RDH_HANDLE handle, char **nac_messageBuffer, int *ac_messageLength) {
  ...
  int rsAnswer ;
  switch (rsAnswer = 
          handle->checkReadFunction(handle->nr_checkReadHandle,
                                    handle->cv_buffer,
                                    handle->checkedReadLength = handle->readLength,
                                    ++(handle->checkReadAttemptsCount),
...
// - RETURNED:
// - -1: unexpected problem
static int WrhSynchronizeFirstMessagePart (WRH_HANDLE handle,
                                           const char *fp_messagePartBuffer, int messagePartLength) {
  m_DIGGY_BOLLARD_S()
  handle->messagePartsNumber = 0;
...
// - RETURNED:
// - -1: unexpected problem
static int WrhSynchronizeNextMessagePart (WRH_HANDLE handle,
                                          const char *fp_messagePartBuffer, int messagePartLength) {
  m_DIGGY_BOLLARD_S()
  int i = handle->messagePartsNumber++ ;
...
// - RETURNED:
// - -1: unexpected problem
static int WrhPrepareWrite (WRH_HANDLE handle, 
                            const char **ap_writeBuffer, int *a_writeLength) {
  m_DIGGY_BOLLARD_S()
...
int ErwCreateInstance (ERW_HANDLE *azh_handle,
                       BROKEN_PIPE_FIX_HANDLE f_brokenPipeFixHandle,
                       ALARM_SYSTEM_HANDLE nf_alarmSystemHandle,
                       int n_readBufferSize,
                       char *cnf_traditionalReadBuffer,
                       CHECK_READ_FUNCTION cn_checkReadFunction,
                       void *ccnfr_checkReadHandle) {
  m_DIGGY_BOLLARD()
  ...
  if (n_readBufferSize != -1) {
    m_TRACK_IF(RdhCreateInstance(&ha_erw->nh_rdhHandle,
                                 n_readBufferSize,
                                 cnf_traditionalReadBuffer,
                                 cn_checkReadFunction,
                                 ccnfr_checkReadHandle) != RETURNED)
  } else {
  ...
  ha_erw->defaultWaitingPlan = p_defaultErwWaitingPlan ;
...
// Public function : see description in .h
int ErwReset (ERW_HANDLE handle, int descriptor,
  ...
  if (nap_defaultWaitingPlan != NULL) {
    handle->defaultWaitingPlan = *nap_defaultWaitingPlan;
  } // if
  handle->n_descriptor = descriptor ;
...
// Public function : see description in .h
int ErwWrite2 (ERW_HANDLE handle,
