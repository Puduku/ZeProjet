...
struct _sps2_PD {
  ...
  struct WAITING_PLAN waitingPlan ; 
  struct {
...
int PdCreateInstance (PD_HANDLE *azh_handle, ALARM_SYSTEM_HANDLE nf_alarmSystemHandle) {
  ...
  (*azh_handle)->waitingPlan = m_WaitingPlan(DEEPLY_BLOCKING__WAITING_PLAN,0);  
...
int PdSetDeadline (PD_HANDLE handle, struct WAITING_PLAN waitingPlan) {
  m_DIGGY_BOLLARD()
  handle->waitingPlan = waitingPlan ;

  if (waitingPlan.waitingPlan == DEEPLY_BLOCKING__WAITING_PLAN &&
      waitingPlan.c_deadline > 0) {
...
int PdSetAlarm (PD_HANDLE handle) {
  m_DIGGY_BOLLARD()
  if (handle->waitingPlan.waitingPlan != NON_BLOCKING__WAITING_PLAN && 
      handle->waitingPlan.c_deadline > 0 && 
      handle->n_alarmSystemHandle != NULL) { 
    if (SetAlarm(handle->n_alarmSystemHandle,
      // assert "ASS" : when c_deadline > 0,
      // handle->waitingPlan.c_deadline - 
      // handle->c_control.alarmDeliveryCorrection > 0
      handle->waitingPlan.c_deadline - handle->c_control.alarmDeliveryCorrection) < 0) {
      m_TRACK()
    } // if
  } // if
...
int PdCheckTimeout (PD_HANDLE handle) {
  ...
  if (handle->waitingPlan.waitingPlan == DEEPLY_BLOCKING__WAITING_PLAN) {
    answer = ANSWER__NO ; // no timeout for the moment...

    if (handle->waitingPlan.c_deadline > 0) {
      time_t now ;

      time(&now) ;
      if (now >= handle->c_control.cc_baseEpochTime + handle->waitingPlan.c_deadline || // timeout
        now < handle->c_control.cc_baseEpochTime) { // system "returns in the past" ! => provoke timeout
        answer = ANSWER__YES ;
      } else {
        // => now < handle->c_control.cc_baseEpochTime + handle.waitingPlan.c_deadline
        ...
        // => handle->c_control.alarmDeliveryCorrection < handle.waitingPlan.c_deadline
...
int PdClearAlarm (PD_HANDLE handle) {
  m_DIGGY_BOLLARD()
  if (handle->waitingPlan.waitingPlan != NON_BLOCKING__WAITING_PLAN &&
      handle->waitingPlan.c_deadline > 0 && handle->n_alarmSystemHandle != NULL) {
