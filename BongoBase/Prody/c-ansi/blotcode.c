...
static inline const char* BlotkeywIdImage(int blotkeywId) {\
  m_ENUM_IMAGE_FUNCTION__OUTSET(blotkeywId)\
  m_ENUM_IMAGE_FUNCTION__BODY_CASE(NONE__BLOTKEYW_ID)\
  m_ENUM_IMAGE_FUNCTION__BODY_CASE(LOOP__BLOTKEYW_ID)\
  m_ENUM_IMAGE_FUNCTION__BODY_CASE(END_LOOP__BLOTKEYW_ID)\
  m_ENUM_IMAGE_FUNCTION__BODY_CASE(SWITCH__BLOTKEYW_ID)\
  m_ENUM_IMAGE_FUNCTION__BODY_CASE(CASE__BLOTKEYW_ID)\
  m_ENUM_IMAGE_FUNCTION__BODY_CASE(DEFAULT_CASE__BLOTKEYW_ID)\
  m_ENUM_IMAGE_FUNCTION__BODY_CASE(END_SWITCH__BLOTKEYW_ID)\
    ...
    if (blotinstPtr->b_blotfunc) {
      ...
      case ANSWER__YES: // Blot function completed
        if (handle->h_blotfuncSurrogate->c_copiedLength > 0) {
          m_TRACK_IF(m_SuckerFillDButt_GString(outputSuckerHandle, handle->h_blotfuncSurrogate,
            NULL) != 0)
        } // if
