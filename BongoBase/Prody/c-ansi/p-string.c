...
( b_FLAGS_OFF(scanFlags,PASS_CHARS_WHILE__SCAN_FLAG) ? !isCharFunction(u_char) : isCharFunction(u_char) )
...
( b_FLAGS_OFF(scanFlags,PASS_CHARS_WHILE__SCAN_FLAG) ? ((char) != (u_char)) : ((char) == u_char) )

  if (b_FLAGS_OFF(scanFlags,REVERTED__SCAN_FLAG)) {
    ptr = pString.string - 1;

// loop invariant: No match in portion [pString.string:ptr[ 
#define m_SCAN_REGULAR(b_passChar) {\
  if (b_FLAGS_ON(scanFlags,QUOTED__SCAN_FLAG)) {\
    ...
    while (++ptr < pString.stop && (b_passChar)) {}\
  }\
} 
...
#define m_SCAN_REVERTED(b_passChar) {\
  if (b_FLAGS_ON(scanFlags,QUOTED__SCAN_FLAG)) {\
    ...
    while (--ptr >= pString.string && (b_passChar)) {}\
  }\
} 
    ptr = pString.stop ;
