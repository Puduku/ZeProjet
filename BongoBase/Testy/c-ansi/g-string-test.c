...
static int GParamTest (const char *p_vers3) {
  ...
  int ret = GStringCopy(h_gParam + G_PARAM_NAME_ELEMENT, 0, o_PString("VERS1")); 
  m_TRACK_IF(ret < 0)
  ret = GStringCopy(h_gParam + G_PARAM_VALUE_ELEMENT, 0, o_PString(VERS1)); 
  m_TRACK_IF(ret < 0)
...
#define     CASE__INDEX_LABEL 1
  ...
  m_TRACK_IF(GStringCopy(gStringStuff,0, o_PString(VERS1)) < 0)

  m_TRACK_IF(GStringsFetch(h_gStrings,  -1,  &gStringStuff) < 0) 
  m_TRACK_IF(GStringCopy(gStringStuff,0, o_PString(VERS2)) < 0)	

  m_TRACK_IF(GStringsFetch(h_gStrings,  -1,  &gStringStuff) < 0)
  m_TRACK_IF(GStringPrintf(gStringStuff,  0,  VERS3_FORMAT_S_S,VERS3_S1,VERS3_S2) < 0)

  m_TRACK_IF(GStringsFetch(h_gStrings,  -1,  &gStringStuff) < 0)
  m_TRACK_IF(GStringCopy(gStringStuff,0, o_PString(LOWER_FOOTER)) < 0)
